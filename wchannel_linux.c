...
*/
wstatus
_wchannel_create_udp(wchannel_opt_t *chan_opt,wchannel_t *channel)
{
	int sock,ecode;
...
wchannel_create(wchannel_opt_t *chan_opt,wchannel_t *channel)
{
	...
	{
		case WCHANNEL_TYPE_SOCKUDP:
			dbgprint(MOD_WCHANNEL,__func__,"calling _wchannel_create_udp function");
			ws = _wchannel_create_udp(chan_opt,&new_channel);
	...
	switch(chan_opt->debug_opts)
	{
		...
		case WCHANNEL_MESSAGE_BUFFER:
			...
			new_channel->message_buffer = (char*)malloc(sizeof(char*)*chan_opt->buffer_size);
			if( !new_channel->message_buffer )
			...
			dbgprint(MOD_WCHANNEL,__func__,"(new_channel=%p) allocated message buffer successfully (p=%p)",
					new_channel,new_channel->message_buffer);
			break;
...
*/
wstatus
wchannel_send(wchannel_t channel,void *buffer_ptr,unsigned int buffer_size,char *dest,unsigned int *buffer_used)
{
