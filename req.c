		...
		unsigned int look_name_size,nvpair_info_t nvpi)
{
	...
	unsigned int nv_idx;
	value_format_list l_value_format;
	nvpair_info_flag_list nvp_flags;
	...
	if( req->stype != REQUEST_STYPE_TEXT ) {
		dbgprint(MOD_MODMGR,__func__,"invalid request to be used with this function (expecting REQUEST_STYPE_TEXT)");
		DBGRET_FAILURE(MOD_MODMGR);
	}
	...
	for( nv_idx = 1 ; ; nv_idx++ )
   	{
		...
		if( ws != WSTATUS_SUCCESS ) {
			dbgprint(MOD_MODMGR,__func__,"(req=%p) failed to validate text request nvpair (idx=%u)",req,nv_idx);
			goto return_fail;
		}

		if( !(nvp_flags & NVPAIR_IFLAG_NAME_VALID) || !(nvp_flags & NVPAIR_IFLAG_VALUE_VALID) ) {
			dbgprint(MOD_MODMGR,__func__,"(req=%p) detected invalid nvpair (idx=%u) in nvpair list, aborting",req,nv_idx);
			goto return_fail;
		}

		/* try to parse nvpair text tokens */
		ws = _req_text_nv_parse(aux,&l_name_ptr,&l_name_size,&l_value_ptr,&l_value_size,&l_value_format);
		if( ws != WSTATUS_SUCCESS ) {
			dbgprint(MOD_MODMGR,__func__,"(req=%p) failed to process text request",req);
			goto return_fail;
		}
		...
		dbgprint(MOD_MODMGR,__func__,"(req=%p) nvpair was found successfully",req);
		nvpi->flags = NVPAIR_IFLAG_FOUND;
		dbgprint(MOD_MODMGR,__func__,"(req=%p) nvpi flags updated to %d (NVPAIR_IFLAG_FOUND)",req,nvpi->flags);

		nvpi->name_ptr = l_name_ptr;
		nvpi->name_size = l_name_size;
		nvpi->value_ptr = l_value_ptr;
		nvpi->value_size = l_value_size;
		nvpi->value_format = l_value_format;

		dbgprint(MOD_MODMGR,__func__,"(req=%p) nvpi updated name_ptr=%p, name_size=%u, value_ptr=%p, value_size=%u, value_format=%d",
				req,nvpi->name_ptr,nvpi->name_size,nvpi->value_ptr,nvpi->value_size,nvpi->value_format);

		nvpi->flags |= NVPAIR_IFLAG_NAME_VALID | NVPAIR_IFLAG_VALUE_VALID;
		dbgprint(MOD_MODMGR,__func__,"(req=%p) nvpi flags updated to %d (|NVPAIR_IFLAG_NAME_VALID | NVPAIR_IFLAG_VALUE_VALID)",req,nvpi->flags);

		if( l_value_format == VALUE_FORMAT_ENCODED )
	   	{
			...
			dbgprint(MOD_MODMGR,__func__,"(req=%p) nvpi updated decoded size value to %u",req,nvpi->decoded_size);
		}
...
seek_nvp:
	...
	nvpi->flags = NVPAIR_IFLAG_NOT_FOUND;
	dbgprint(MOD_MODMGR,__func__,"(req=%p) nvpi updated flags to %d (NVPAIR_IFLAG_NOT_FOUND)",req,nvpi->flags);
